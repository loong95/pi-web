...
- `get_subagent_result({ wait: true })` and `subagentFinalText()` share one `UNFINISHED_SUBAGENT_STATUSES` set (`queued`, `starting`, `running`). A run waiting for its parent's `maxConcurrent` slot sits in `queued` for as long as its siblings run, so a wait loop that stopped at `starting`/`running` returned instantly *and* the text fell through to the failure line: the model was told a merely-queued subagent had "failed: Unknown error" — with no `error` field to recover from — and would retry the run or report a failure that never happened. Both call sites must read the same set; the wait loop's unbounded poll is the existing contract, so covering `queued` adds queue delay, not a new class of wait.
