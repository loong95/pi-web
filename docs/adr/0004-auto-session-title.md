...
`setSessionName`. The title request itself goes to a temporary `Agent` that
copies the source session's system prompt, tools, and messages with every tool
shadowed to throw, so a naming run can neither mutate the project nor appear in
the session file.
...
task. Reusing the session's own model keeps the provider cache prefix the manual
flow relies on; choosing another model is an explicit tradeoff for cost or
latency, and `null` (the default) means "follow the session".
