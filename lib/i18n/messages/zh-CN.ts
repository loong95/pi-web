...
export const zhCNLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "settings.plantUmlDescription": "使用您自托管的 PlantUML Server 渲染 PlantUML 代码块；默认不会使用公共服务器。",
    "settings.plantUmlServerUrl": "PlantUML Server URL",
    "settings.testPlantUmlConnection": "测试连接",
    "settings.testingPlantUml": "正在测试连接…",
    "settings.clearPlantUmlServer": "清除服务器",
    ...
    "settings.experimentalDescription": "仍在摸索中的功能，可能随时变更或移除。",
    "settings.worktreeSessions": "会话列表",
    "settings.onlyCurrentWorktree": "只显示当前 worktree 的会话",
    "settings.onlyCurrentWorktreeDescription": "在侧边栏隐藏本项目其他 worktree 的会话，对所有项目生效。",
    "settings.autoTitle": "自动生成会话标题",
    "settings.autoTitleDescription": "实验性功能：会话的首条消息完成后在后台生成标题。生成失败会被忽略，不影响对话，标题栏的手动生成按钮仍然可用。",
    "settings.autoTitleEnabled": "首条消息后自动生成标题",
    "settings.autoTitleModel": "标题模型",
    "settings.autoTitleModelDescription": "自动和手动生成标题都使用该模型。留空则复用会话当前使用的模型；所选模型不可用时同样回退到该模型。",
    "settings.autoTitleModelSession": "跟随会话模型",
    "settings.autoTitleModelLoading": "正在加载模型…",
    "settings.autoTitleModelUnavailable": "{model}（不可用）",
    "settings.autoTitleLoadFailed": "无法读取标题设置：{message}",
    "settings.autoTitleSaveFailed": "无法保存标题设置：{message}",
    "auth.prompt": "登录后继续使用",
    "auth.password": "密码",
    "auth.logIn": "登录",
    "auth.loggingIn": "正在登录...",
    ...
    "i18n.showPlantUmlSource": "显示 PlantUML 源码",
    "i18n.previewPlantUml": "预览 PlantUML 图表",
    "i18n.renderingPlantUml": "正在渲染 PlantUML 图表",
    "i18n.plantUmlRenderFailed": "PlantUML 渲染失败",
    "i18n.openPlantUmlViewer": "打开图表查看器",
    "i18n.plantUmlViewer": "PlantUML 图表查看器",
    "i18n.plantUmlDiagram": "PlantUML 图表",
