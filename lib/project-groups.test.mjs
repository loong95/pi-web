...
test("worktree activity aggregates per checkout and ignores sessions without one", () => {
  const main = { ...session("main", "/repo", "2026-08-12T00:00:00.000Z"), worktreeKey: "/repo" };
  const feature = { ...session("feature", "/repo", "2026-08-12T00:01:00.000Z"), worktreeKey: "/repo-worktrees/feature" };
  const removed = session("removed", "/repo", "2026-08-12T00:02:00.000Z");

  const activity = getWorktreeActivity(
    [main, feature, removed],
    new Set(["feature"]),
    new Set(["feature", "removed"]),
  );

  assert.deepEqual(activity.get("/repo"), { running: 0, unread: 0 });
