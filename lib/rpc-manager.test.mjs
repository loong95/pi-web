...
test("serializes custom notifications behind prompt admission", async () => {
  const manager = SessionManager.inMemory(tmpdir());
  let signalPreflight;
  const preflightStarted = new Promise((resolve) => { signalPreflight = resolve; });
  let finishPrompt;
  ...
  try {
    const userPrompt = wrapper.send({ type: "prompt", message: "user request" });
    await preflightStarted;
    let notificationSettled = false;
    const notification = wrapper.sendCustomMessage({
      customType: "pi-web:subagent-notification",
      content: "subagent result",
      display: true,
    }, { deliverAs: "followUp", triggerTurn: true }).then(() => {
