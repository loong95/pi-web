...
test("a model override replaces the model and the thinking level", () => {
  const sessionModel = { provider: "session", id: "session-model" };
  const overrideModel = { provider: "title", id: "title-model", reasoning: true };
  const source = {
    state: {
      systemPrompt: "system",
      model: sessionModel,
  ...
  assert.equal(
    buildSessionTitleAgentOptions(source).initialState.model,
    sessionModel,
  );
});

function overrideSource(modelForPair) {
  const sessionModel = { provider: "session", id: "session-model" };
  const calls = {};
  const sourceAgent = {
    state: {
    ...
    streamFunction: (model, context, options) => {
      ...
      calls.roles = context.messages.map((message) => message.role);
      calls.lastText = String(context.messages.at(-1)?.content);
      const stream = createAssistantMessageEventStream();
      queueMicrotask(() => {
        stream.push({
          type: "done",
          reason: "stop",
          message: assistantMessage("Cheap Title Model"),
        });
      });
      return stream;
    },
  ...
  const source = {
    agent: sourceAgent,
    modelRuntime: {
      getModel: (provider, modelId) => modelForPair(provider, modelId),
      refresh: async () => {},
...
test("generates the title with the configured model and no thinking", async () => {
  const overrideModel = { provider: "title", id: "title-model", reasoning: true };
  const { calls, source } = overrideSource((provider, modelId) => (
...
test("an unusable configured title model falls back to the session model", async () => {
  const missing = overrideSource(() => undefined);
  ...
  assert.equal(missing.calls.model, missing.source.agent.state.model);
  ...
  const unauthenticated = overrideSource(() => ({ provider: "title", id: "title-model" }));
  ...
  assert.equal(unauthenticated.calls.model, unauthenticated.source.agent.state.model);
