...
test("automatic naming only names a first-message session once", () => {
  assert.equal(resolveAutoTitleSkipReason({ enabled: false, userMessages: 1 }), "disabled");
  assert.equal(resolveAutoTitleSkipReason({ enabled: true, userMessages: 0 }), "no-user-message");
  assert.equal(resolveAutoTitleSkipReason({ enabled: true, userMessages: 2 }), "not-first-message");
  assert.equal(resolveAutoTitleSkipReason({ enabled: true, sessionName: "Kept", userMessages: 1 }), "already-named");
  assert.equal(resolveAutoTitleSkipReason({ enabled: true, sessionName: "  ", userMessages: 1 }), null);
  assert.equal(resolveAutoTitleSkipReason({ enabled: true, userMessages: 1 }), null);
});

function titleOverrideSource(resolveModel, title = "Cheap Title Model") {
  const sessionModel = { provider: "session", id: "session-model", reasoning: false };
  const calls = {};
  const sourceAgent = {
    state: {
      systemPrompt: "system",
    ...
    streamFunction: (model, context, options) => {
      calls.model = model;
      calls.reasoning = options?.reasoning;
      calls.messages = context.messages;
      const stream = createAssistantMessageEventStream();
      queueMicrotask(() => {
        stream.push({ type: "done", reason: "stop", message: assistantMessage(title) });
      });
  ...
  const source = {
    agent: sourceAgent,
    modelRuntime: {
      getModel: (provider, modelId) => resolveModel(provider, modelId),
      refresh: async () => {},
      hasConfiguredAuth: () => true,
    },
  };
  return { calls, source, sessionModel };
}

test("names the session with the configured title model", async () => {
  const overrideModel = { provider: "title", id: "title-model", reasoning: false };
  const { calls, source } = titleOverrideSource((provider, modelId) => (
    provider === "title" && modelId === "title-model" ? overrideModel : undefined
  ));

  const result = await generateSessionTitle(source, {
    model: { provider: "title", modelId: "title-model" },
  });

  assert.equal(calls.model, overrideModel);
  // A title is extraction, not reasoning: an override never inherits the session's thinking level.
  assert.equal(calls.reasoning, undefined);
  assert.match(JSON.stringify(calls.messages.at(-1).content), /Name this session/);
  assert.deepEqual(result.model, { provider: "title", modelId: "title-model" });
  assert.equal(result.modelFallback, undefined);
});

test("clamps a reasoning title model to its cheapest thinking level", async () => {
  const overrideModel = { provider: "title", id: "gemini-2.5-flash", api: "google-generative-ai", reasoning: true };
  const { calls, source } = titleOverrideSource(() => overrideModel);

  await generateSessionTitle(source, { model: { provider: "title", modelId: "gemini-2.5-flash" } });

  assert.equal(calls.reasoning, "low");
});

test("an unusable configured title model falls back to the session model", async () => {
  const missing = titleOverrideSource(() => undefined);
  const missingResult = await generateSessionTitle(missing.source, {
    model: { provider: "title", modelId: "gone" },
  });

  assert.equal(missingResult.title, "Cheap Title Model");
  assert.equal(missing.calls.model, missing.sessionModel);
  assert.deepEqual(missingResult.model, { provider: "session", modelId: "session-model" });
  assert.deepEqual(missingResult.modelFallback, {
    requested: { provider: "title", modelId: "gone" },
  });

  const unauthenticated = titleOverrideSource(() => ({ provider: "title", id: "title-model", reasoning: false }));
  unauthenticated.source.modelRuntime.hasConfiguredAuth = () => false;
  const unauthenticatedResult = await generateSessionTitle(unauthenticated.source, {
    model: { provider: "title", modelId: "title-model" },
  });

  assert.equal(unauthenticated.calls.model, unauthenticated.sessionModel);
  assert.deepEqual(unauthenticatedResult.modelFallback, {
    requested: { provider: "title", modelId: "title-model" },
  });
});
