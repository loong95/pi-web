...
test("get_subagent_result covers missing, queued, running, completed, failed, and aborted runs", async () => {
  let current = null;
  const { tools } = await loadTools({
    async start() { throw new Error("unused"); },
  ...
  // A run waiting for a `maxConcurrent` slot has not started, let alone failed: the model
  // reads this text as the state of the run, so it must not fall through to the failure line.
...
test("get_subagent_result wait also covers a run that is still queued", async () => {
  ...
  let settled = false;
  const pending = tools.get("get_subagent_result")
    .execute("call", { agent_id: "child-session", wait: true })
    .then((value) => { settled = true; return value; });

  // One poll interval passes without the run leaving the queue.
  await new Promise((resolve) => setTimeout(resolve, 700));
  assert.equal(settled, false, "wait must keep polling while the run is queued");

  current = run({ status: "completed", result: "queued result" });
