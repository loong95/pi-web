...
test("background completion keeps its subagent provenance at the provider boundary", () => {
  const result = convertToLlm([
    { role: "user", content: "Inspect the parser", timestamp: Date.now() },
    {
      role: "custom",
      customType: "pi-web:subagent-notification",
      content: subagentNotificationText(run({ status: "completed", result: "Parser found" })),
      display: true,
      details: { sessionId: "child-session" },
      timestamp: Date.now(),
    },
  ]);
  ...
  assert.match(result[1].content[0].text, /asynchronous subagent result/);
  assert.match(result[1].content[0].text, /not a new user request/);
  assert.match(result[1].content[0].text, /<subagent_result>[\s\S]*Parser found[\s\S]*<\/subagent_result>/);
});
...
test("get_subagent_result acknowledges terminal background results", async () => {
  let current = run({ runInBackground: true, status: "completed", result: "done" });
  const acknowledged = [];
  const { tools } = await loadTools({
    async start() { throw new Error("unused"); },
    async get() { return current; },
  ...
  assert.equal(result.content[0].text, "done");
  assert.deepEqual(acknowledged, ["child-session"]);

  current = run({ runInBackground: true, status: "running" });
  await tools.get("get_subagent_result").execute("call", { agent_id: "child-session" });
  assert.deepEqual(acknowledged, ["child-session"]);
});

test("get_subagent_result wait includes queued runs before acknowledging", async () => {
  let calls = 0;
  const acknowledged = [];
  const { tools } = await loadTools({
    async start() { throw new Error("unused"); },
    async get() {
      calls += 1;
      return calls === 1
        ? run({ runInBackground: true, status: "queued" })
        : run({ runInBackground: true, status: "completed", result: "done" });
    },
    async acknowledge(id) { acknowledged.push(id); },
    async steer() {},
  });
