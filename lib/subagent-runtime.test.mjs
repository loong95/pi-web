...
test("completion notification reopens an idle parent and uses its current session", async () => {
  ...
  assert.match(delivered[0].message.content, /Pi Web asynchronous subagent result/);
  assert.match(delivered[0].message.content, /Parser found/);
...
test("delayed completion notification is suppressed after explicit result retrieval", async () => {
  const previousStates = globalThis.__piSubagentNotificationStates;
  globalThis.__piSubagentNotificationStates = new Map([
    ["child-session\u0000tool-call", { acknowledged: false, delivered: false }],
  ]);
  const delivered = [];
  let releaseIdle;
  let signalIdle;
  ...
  const parent = {
    ...
    waitUntilIdle: () => new Promise((resolve) => {
      signalIdle();
      releaseIdle = resolve;
    }),
    sendCustomMessage: async (message, options, shouldSend) => {
      if (shouldSend?.() ?? true) delivered.push({ message, options });
    },
    inner: {},
  };
  const controller = createSubagentController({
    ...
    resolveSessionPath: async () => parent.sessionFile,
    invalidateSessionList: () => {},
  });

  try {
    const notification = controller.extensionRuntime.notifyParent(completedRun());
    await idleStarted;
    await controller.extensionRuntime.acknowledge("child-session", "tool-call");
    releaseIdle();
    await notification;
    assert.equal(delivered.length, 0);
  } finally {
    if (previousStates === undefined) delete globalThis.__piSubagentNotificationStates;
    else globalThis.__piSubagentNotificationStates = previousStates;
  }
});

test("explicit result retrieval suppresses an already-waiting notification without an in-memory claim", async () => {
  const previousStates = globalThis.__piSubagentNotificationStates;
  delete globalThis.__piSubagentNotificationStates;
  const delivered = [];
  let releaseIdle;
  let signalIdle;
  const idleStarted = new Promise((resolve) => { signalIdle = resolve; });
