...
test("main and linked worktrees share one canonical project root", async (t) => {
  ...
  const { createJiti } = await import("jiti");
  const { projectIdentityKey: resolveProjectIdentityKey } = await createJiti(import.meta.url).import("./project-identity.ts");
  ...
  // One project, two distinct checkout identities for worktree scoping.
  assert.equal(mainProject.worktreePath, realpathSync(repo));
  assert.equal(linkedProject.worktreePath, realpathSync(linked));
  assert.notEqual(mainProject.worktreePath, linkedProject.worktreePath);
  ...
  assert.equal(listedLinked.key, resolveProjectIdentityKey(listedLinked.path));
  assert.notEqual(listedLinked.key, worktrees.find((worktree) => worktree.isMain)?.key);
